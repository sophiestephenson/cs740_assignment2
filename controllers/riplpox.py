...
class RipLController(object):
    ...
    def _install_hybrid_dynamic_flows(self, event, out_dpid, final_out_port, packet):
        ...
        if len(src_dst_route) == 1:
            ...
            log.info(
                "adding edge-only entry from %s to %s on sw %s"
                % (match.dl_src, match.dl_dst, in_name)
            )
            self.switches[event.dpid].install(
                final_out_port,
                match,
                    ...
                    if i == 0:
                        ...
                        if i == 1:
                            ...
                            self.switches[node_dpid].install(
                                src_port, match, priority=PRIO_HYBRID_VLAN_DOWN
                            )
